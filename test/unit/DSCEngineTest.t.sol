...
contract DSCEngineTest is StdCheats, Test {
    ...
    function testRevertsWithUnapprovedCollateral() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                DSCEngine.DSCEngine__TokenAddressNotValid.selector,
                address(randToken)
            )
        );
    ...
    function testRevertsIfMintedDscBreaksHealthFactor() public {
        ...
        amountToMint =
            (amountCollateral *
                (uint256(price) * dsce.get_ADDITIONAL_FEED_PRECISION())) /
            dsce.get_PRECISION();
        vm.startPrank(user);
        ERC20Mock(weth).approve(address(dsce), amountCollateral);
    ...
    //     mockDsce.depositCollateralAndMintSc(
    //         weth,
    //         amountCollateral,
    //         amountToMint
    //     );
    ...
    /**
     * @dev this test is wrongly set, it reverts DSCEngine__BreaksHealthFactor(5000)
     * that's because of the `depositCollateralAndMintSc` function, exactly the mintSc function
     * which runs `_revertIfHealthFactorIsBroken`
     */

    function testRevertsIfRedeemAmountIsZero() public {
        vm.startPrank(user);
        ERC20Mock(weth).approve(address(dsce), amountCollateral);
        dsce.depositCollateralAndMintSc(
            address(weth),
            amountCollateral,
            amountToMint
        );
    ...
    function testGetMinHealthFactor() public {
        uint256 minHealthFactor = dsce.get_MIN_HEALTH_FACTOR();
        assertEq(minHealthFactor, MIN_HEALTH_FACTOR);
    }

    function testGetLiquidationThreshold() public {
        uint256 liquidationThreshold = dsce.get_LIQUIDATION_THRESHOLD();
    ...
    function testGetCollateralBalanceOfUser() public {
        ...
        dsce.depositCollateral(weth, amountCollateral);
        vm.stopPrank();
        uint256 collateralBalance = dsce.getCollateralAmountDeposited(
            user,
            weth
        );
    ...
    function testGetDsc() public {
        address dscAddress = dsce.get_Sc();
        assertEq(dscAddress, address(dsc));
    }

    function testLiquidationPrecision() public {
        uint256 expectedLiquidationPrecision = 100;
        uint256 actualLiquidationPrecision = dsce.get_LIQUIDATION_PRECISION();
